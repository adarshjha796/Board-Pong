...
public class Launcher2 : MonoBehaviour
{
    ...
    private static Launcher2 _instance;
    ...
    public static Launcher2 Instance
    {
        get
        {
            if (_instance == null)
            {
                GameObject go = new GameObject("Launcher2");
                go.AddComponent<Launcher2>();
            }
            return _instance;
        }
    }
        ...
        if (/*PV.IsMine*/ true)
        {
            if (inTrigger) // This will happen when the ball is in touch with the launcher.
            {
                ...
                if (Input.GetMouseButton(0)) // Mouse when held down.
                {
                    ...
                    currentPoint.z = 0;
                    TrajectoryLine.Instance.RenderLine(BallManager.Instance.renderLines[1].position, currentPoint); // Drawing the line.
                }
    ...
    {
        if (collision.gameObject.name == "Ball 2(Clone)" && BallManager.Instance.selectedObject == collision.gameObject)
        {
            inTrigger = true;
        }
    }
    ...
    {
        if (collision.gameObject.name == "Ball 2(Clone)")
        {
